...
export default class CircularMenu {
  constructor({ container, vrIntegration, onAudioToggle, onWalkMode, onFullscreen, onCancelGame }) {
    ...
    // HUD: Nivel
    this.levelStatus = document.createElement('div')
    this.levelStatus.id = 'hud-level'
    this.levelStatus.innerText = '🗺️ Nivel: 1' // Valor inicial
    Object.assign(this.levelStatus.style, {
      position: 'fixed',
      top: '16px',
    ...
    // ✨ CORRECCIÓN: Añadir al body, no al container
    document.body.appendChild(this.levelStatus)
    ...
    Object.assign(this.timer.style, {
      position: 'fixed',
      top: '16px', // Misma altura que el nivel
      left: '150px', // ✨ Ajustar posición para no solaparse
      fontSize: '16px',
      fontWeight: 'bold',
      background: 'rgba(0,0,0,0.6)',
      color: 'white',
    ...
    Object.assign(this.playersLabel.style, {
      position: 'fixed',
      top: '16px',
      left: '260px', // ✨ Ajustar posición para no solaparse
      fontSize: '16px',
      fontWeight: 'bold',
      background: 'rgba(0,0,0,0.6)',
      color: 'white',
  ...
  setLevelStatus(level) {
    if (this.levelStatus) this.levelStatus.innerText = `🗺️ Nivel: ${level}`
  }

...
export default class Robot {
    ...
    update() {
        ...
        if (keys.down) {
            const backward = new THREE.Vector3(0, 0, -6)
    ...
    die() {
        ...
        // ❌ NO eliminar el cuerpo físico, solo desactivarlo temporalmente
        if (this.body) {
            this.body.velocity.set(0, 0, 0)
            this.body.angularVelocity.set(0, 0, 0)
            this.body.sleep() // Poner a dormir en lugar de eliminar
        }
        ...
        console.log('💀 Robot ha muerto (pero puede resucitar)')
    }

    reset() {
        // 💀 Si está muerto, reactivar el cuerpo físico
        if (!this.body) {
            this.setPhysics()
        } else {
            // Reactivar el cuerpo si existe pero está desactivado
            this.body.wakeUp()
            this.body.position.set(0, 1.2, 0)
            this.body.velocity.set(0, 0, 0)
            this.body.angularVelocity.set(0, 0, 0)
        ...
        console.log('🤖 Robot reseteado correctamente')
    }
}
